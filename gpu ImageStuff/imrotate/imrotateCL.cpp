...
int main(int argc, char *argv[]){
	...
	if ( argc != 5 ) {
		...
		printf("Usage: %s <input image file name> <output image file name> <number of rotations> <device number>\n", argv[0]);
		exit(EXIT_FAILURE);
	}

	int device = atoi(argv[4]);
	int NumRot = atoi(argv[3]);
	...
	FILE *fp;
	char fileName[] = "./imrotate.cl"; // .cl file name here
	char *source_str;
	size_t source_size;
    ...
    kernel = clCreateKernel(program, "rotate_kernel", &err);
	...
	err |= clSetKernelArg(kernel, 5, sizeof(int), &NumRot);
    if (err != CL_SUCCESS){
	...
	sprintf(output_filename,"%sAROT%d.bmp", argv[2], i);
