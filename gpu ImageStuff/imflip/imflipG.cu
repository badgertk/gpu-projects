...
__global__ void FlipImageH(uchar *GPU_i, uchar *GPU_o, int M, int N)
{
...
cudaError_t launch_helper(uchar **CPU_InputArray, uchar **CPU_OutputArray, float* Runtimes)
{
  ...
  if ((GPU_idata == NULL) || (GPU_odata == NULL)) {
    ...
    return(cudaErrorUnknown);  // could do cudaErrorMemoryAllocation, but we're not really here due to a CUDA error
  }
	// Create events to keep track of execution time
  ...
  cudaEventRecord(time2, 0);
    // Launch kernel:
    FlipImageH<<<numBlocks, threadsPerBlock>>>(GPU_idata[i], GPU_odata[i], M, N);
