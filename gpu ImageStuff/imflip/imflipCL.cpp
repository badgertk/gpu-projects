...
int main(int argc, char *argv[]){
	...
	cl_program program;
	cl_kernel kernel;
	cl_event event2; // to keep track of execution times
	...
	FILE *fp;
	char fileName[] = "./imflip.cl"; // .cl file name here
	char *source_str;
	size_t source_size;
	...
	int M = image.rows;
	int N = image.cols;
	size_t TotalSize = M * N * sizeof(unsigned char) * 3;
    if(! image.data ){
    ...
    kernel = clCreateKernel(program, "FlipImageH", &err);
    ...
    input = clCreateBuffer(context,  CL_MEM_READ_ONLY,  TotalSize, NULL, NULL);
    ...
    // Execute the kernel over the entire range of our 1d input data set
    // using the maximum number of work group items for this device
    err = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, &global, &local, 0, NULL, &event2);
    if (err){
	...
	Mat result = Mat(M, N, CV_8UC3, CPU_OutputArray);
	...
	printf("\nSaved image '%s', size = %dx%d (dims = %d).\n", argv[2], result.rows, result.cols, result.dims);
	...
	float TotalExecTime = HtoD_time + GPUTime + DtoH_time;
