  ...
  int points_per_hat)
{
  ...
  cross_correlate_point_with_wavelet(&out_signal[idx], in_signal, hat_signal, idx, signal_size, points_per_hat);
}
  ...
  int num_points)
{
  ...
  if (idx == (num_points - 1)) {
    out_signal[idx] = 0;
  } else if (!in_signal[idx] && in_signal[idx+1]) {
    out_signal[idx] = idx;
  } else {
    out_signal[idx] = 0;
  }
...
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  int which_block = idx / 64; // idx / 32
  out_signal[which_block] = 0;
  if (idx == (num_points - 1)) {
    return;
  } else if (!in_signal[idx] && in_signal[idx+1]) {
    out_signal[which_block] = idx;
  }
}
...
{
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  out_signal[tid] = (60 * sampling_rate) / (sig1[tid] - sig2[tid]);
}
...
{
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  // Division / remainder no more expensive than the shift / and
...
// Run with a smaller thread/block size than the others
__global__ void
get_rr(
  int * out_signal,
  int * in_signal,
  ...
  int sampling_rate)
{
  int idx = (threadIdx.x + blockIdx.x * blockDim.x) * window_size;
  int i;
  int end = idx + window_size*2;
  int prev = 0;
  for (i = idx; i < end; i ++) {
    if (in_signal[i]) {
      if (prev) {
        out_signal[i] = (sampling_rate * 60)/(i - prev);
      }
      prev = i;
    }
  }
