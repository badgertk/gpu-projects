...
void *MTFlipVM3(void* tid){//bagged and tagged
    ...
    for(row=ts; row<=te - simulrow + 1; row = row + simulrow){
...
int main(int argc, char** argv)
{
    ...
    switch (argc){
		case 5: NumThreads = atoi(argv[3]);
				version = atoi(argv[4]);//version 0 - 3 or W
				break;
/* 		default: printf(); //usage not right
		...
		default: printf("\n\nUsage: imflipPMC infile.bmp outfile.bmp numThreads [version]\n");
				 printf("Use 0 for the memory-friendly version of the program.  Use 1 for the memory and core-friendly version of the program.\n");
	...
	switch (version){
		...
		case 3:	 MTFlipFunc = MTFlipVM3; FlipFunc=FlipImageV;	strcpy(FlipType,"Vertical"); 
				 break;
		case 'W':MTFlipFunc = MTFlipVM;	 FlipFunc=FlipImageV;	strcpy(FlipType,"Vertical");
