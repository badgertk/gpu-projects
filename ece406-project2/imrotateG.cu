...
__global__ void rotate_kernel(uchar *GPU_i, uchar *GPU_o, int M, int N, int i, int j){
	...
	row2=v-(int)newY;
/*	if (X < -99){
		printf("col = %d, X = %g, newX = %g, col2 = %d\n", col, X, newX, col2);
		//printf("Y = %g, newY = %g, row2 = %d\n", Y, newY, row2);
		//printf("FAILED!\n");
	}*/
	//if((col2<N) && (row2<M)){
		// maps old pixel to new pixel
		int idx2 = row2*N + col2;
					//GPU_o[idx2] = white; //start by making each pixel white
		//CopyImage[row2][col2]   = TheImage[row][col];
		//idx2 = idx;
	if (idx == 1500){
	printf("CCCC%u", output);
	}
	GPU_o[idx2] = output;
	/*if (i <= 2){
		GPU_o[idx2] = output;
